...
resource "aws_apigatewayv2_integration" "ai_advice_integration" {
  api_id             = aws_apigatewayv2_api.sat_api.id
  integration_type   = "AWS_PROXY"
  integration_uri    = aws_lambda_function.ai_advice.invoke_arn
  integration_method = "POST"
}

resource "aws_apigatewayv2_route" "ai_advice_route" {
  api_id             = aws_apigatewayv2_api.sat_api.id
  route_key          = "POST /ai-advice"
  target             = "integrations/${aws_apigatewayv2_integration.ai_advice_integration.id}"
  authorization_type = "JWT"
  authorizer_id      = aws_apigatewayv2_authorizer.cognito.id
}

resource "aws_lambda_permission" "apigw_ai_advice" {
  statement_id  = "AllowExecutionFromAPIGatewayAIAdvice"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.ai_advice.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.sat_api.execution_arn}/*/*"
}
