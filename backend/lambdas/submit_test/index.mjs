...
export const handler = async (event) => {
  try {
    ...
    testItem.questions.forEach((q) => {
      ...
      let isCorrect = false;
      if (correctKey.length === 1 && ['a', 'b', 'c', 'd'].includes(correctKey)) {
        // It's a multiple choice question. The frontend might send "A. 47" or "A"
        isCorrect = normChosen.charAt(0) === correctKey;
      } else {
        // It's a grid-in question
        isCorrect = normChosen === correctKey;
      }
