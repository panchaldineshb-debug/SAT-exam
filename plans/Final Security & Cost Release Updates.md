...
1. **Enabled DynamoDB Point-in-Time Recovery:**
   - Applied to all 6 tables (`users`, `tests`, `progress`, `activity_log`, `reviews`, `aggregates`). Student's progress is now constantly backed up and can be restored to any second within a 35-day window.
