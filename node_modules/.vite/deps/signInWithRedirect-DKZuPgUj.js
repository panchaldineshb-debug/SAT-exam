...
var logger$8 = new ConsoleLogger("Hub");
var HubClass = class {
	constructor(name) {
	...
	*/
	_remove(channel, listener) {
		...
		if (!holder) {
			logger$8.warn(`No listeners for ${channel}`);
			return;
		}
	...
	dispatch(channel, payload, source, ampSymbol) {
		if (typeof channel === "string" && this.protectedChannels.indexOf(channel) > -1) {
			if (!(ampSymbol === AMPLIFY_SYMBOL)) logger$8.warn(`WARNING: ${channel} is protected and dispatching on it can have unintended consequences`);
		}
		const capsule = {
			channel,
			payload: { ...payload },
	...
	_toListeners(capsule) {
		...
		if (holder) holder.forEach((listener) => {
			logger$8.debug(`Dispatching to ${channel} with `, payload);
			try {
...
var logger$7 = new ConsoleLogger("parseAWSExports");
var authTypeMapping = {
	API_KEY: "apiKey",
	AWS_IAM: "iam",
...
var parseAWSExports = (config = {}) => {
	...
	if (aws_appsync_graphqlEndpoint) {
		...
		if (!defaultAuthMode) logger$7.debug(`Invalid authentication type ${aws_appsync_authenticationType}. Falling back to IAM.`);
...
function parseNotifications(amplifyOutputsNotificationsProperties) {
	...
	if (!(hasInAppMessaging || hasPushNotification || !!amazon_connect)) return;
	const notificationsConfig = {};
	if (hasInAppMessaging && amazon_pinpoint_app_id && aws_region) notificationsConfig.InAppMessaging = { Pinpoint: {
		appId: amazon_pinpoint_app_id,
		region: aws_region
	} };
	const pushNotificationConfig = {};
	if (hasPushNotification && amazon_pinpoint_app_id && aws_region) pushNotificationConfig.Pinpoint = {
		appId: amazon_pinpoint_app_id,
		region: aws_region
	};
	if (amazon_connect) pushNotificationConfig.CustomerProfiles = {
		endpoint: amazon_connect.endpoint,
		region: amazon_connect.aws_region
	};
	if (Object.keys(pushNotificationConfig).length > 0) notificationsConfig.PushNotification = pushNotificationConfig;
...
var logger$6 = new ConsoleLogger("Auth");
var AuthClass = class {
	/**
	...
	*/
	configure(authResourcesConfig, authOptions) {
		...
		if (authResourcesConfig && authResourcesConfig.Cognito?.userPoolEndpoint) logger$6.warn(getCustomEndpointWarningMessage("Amazon Cognito User Pool"));
		if (authResourcesConfig && authResourcesConfig.Cognito?.identityPoolEndpoint) logger$6.warn(getCustomEndpointWarningMessage("Amazon Cognito Identity Pool"));
	}
	/**
...
*/
var logger$5 = new ConsoleLogger("CoreStorageUtils");
...
var logger$4 = new ConsoleLogger("StorageCache");
/**
...
var StorageCacheCommon = class {
	...
	*/
	configure(config) {
		if (config) {
			if (config.keyPrefix) logger$4.warn("keyPrefix can not be re-configured on an existing Cache instance.");
			this.config = {
				...this.config,
				...config
			};
	...
	*/
	async setItem(key, value, options) {
		logger$4.debug(`Set item: key is ${key}, value is ${value} with options: ${options}`);
		if (!key || key === "CurSize") {
			logger$4.warn(`Invalid key: should not be empty or reserved key: '${currentSizeKey}'`);
			return;
		}
		if (typeof value === "undefined") {
			logger$4.warn(`The value of item should not be undefined!`);
			return;
		}
		...
		if (cacheItemOptions.priority < 1 || cacheItemOptions.priority > 5) {
			logger$4.warn(`Invalid parameter: priority due to out or range. It should be within 1 and 5.`);
			return;
		}
		const prefixedKey = `${this.config.keyPrefix}${key}`;
		const item = this.fillCacheItem(prefixedKey, value, cacheItemOptions);
		if (item.byteSize > this.config.itemMaxSize) {
			logger$4.warn(`Item with key: ${key} you are trying to put into is too big!`);
			return;
		}
		try {
			...
			logger$4.warn(`setItem failed! ${e}`);
		}
	}
	/**
	...
	*/
	async getItem(key, options) {
		logger$4.debug(`Get item: key is ${key} with options ${options}`);
		let cached;
		if (!key || key === "CurSize") {
			logger$4.warn(`Invalid key: should not be empty or reserved key: '${currentSizeKey}'`);
			return null;
		...
		try {
			...
			logger$4.warn(`getItem failed! ${e}`);
			return null;
		}
	}
	/**
	...
	*/
	async removeItem(key) {
		logger$4.debug(`Remove item: key is ${key}`);
		if (!key || key === "CurSize") {
			logger$4.warn(`Invalid key: should not be empty or reserved key: '${currentSizeKey}'`);
			return;
		}
		...
		try {
			...
			logger$4.warn(`removeItem failed! ${e}`);
		}
	}
	/**
	...
	*/
	async getAllKeys() {
		try {
			...
			logger$4.warn(`getAllkeys failed! ${e}`);
			return [];
		}
	}
	getStorage() {
	...
	*/
	async removeCacheItem(prefixedKey, size) {
		...
		try {
			...
			logger$4.error(`Failed to remove item: ${removeItemError}`);
		}
	}
	/**
	...
	*/
	...
	sanitizeConfig() {
		if (this.config.itemMaxSize > this.config.capacityInBytes) {
			logger$4.error("Invalid parameter: itemMaxSize. It should be smaller than capacityInBytes. Setting back to default.");
			this.config.itemMaxSize = defaultConfig.itemMaxSize;
		}
		if (this.config.defaultPriority > 5 || this.config.defaultPriority < 1) {
			logger$4.error("Invalid parameter: defaultPriority. It should be between 1 and 5. Setting back to default.");
			this.config.defaultPriority = defaultConfig.defaultPriority;
		}
		if (Number(this.config.warningThreshold) > 1 || Number(this.config.warningThreshold) < 0) {
			logger$4.error("Invalid parameter: warningThreshold. It should be between 0 and 1. Setting back to default.");
			this.config.warningThreshold = defaultConfig.warningThreshold;
		}
		if (this.config.capacityInBytes > 5 * 1024 * 1024) {
			logger$4.error("Cache Capacity should be less than 5MB. Setting back to default. Setting back to default.");
	...
	*/
	async setCacheItem(prefixedKey, item) {
		...
		try {
			...
			logger$4.error(`Failed to set item ${setItemErr}`);
		}
	}
	/**
	...
	*/
	async clear() {
		...
		try {
			...
			logger$4.warn(`clear failed! ${e}`);
		}
	}
};
//#endregion
//#region node_modules/@aws-amplify/core/dist/esm/Cache/StorageCache.mjs
var logger$3 = new ConsoleLogger("StorageCache");
	...
	*/
	createInstance(config) {
		if (!config.keyPrefix || config.keyPrefix === defaultConfig.keyPrefix) {
			logger$3.error("invalid keyPrefix, setting keyPrefix with timeStamp");
...
var logger$2 = new ConsoleLogger("I18n");
/**
...
var logger$1 = new ConsoleLogger("I18n");
...
var I18n = class I18n {
	...
	*/
	static createInstance() {
		logger$1.debug("create I18n instance");
		if (_i18n) return;
		_i18n = new I18n$1();
	}
	/**
...
//#endregion
export { assertIdTokenInAuthTokens as $, resetActiveSignInState as A, InAppMessagingAction as At, createAssociateSoftwareTokenClient as B, AMPLIFY_SYMBOL as Bt, serializePkcWithAttestationToJson as C, Amplify as Ct, PasskeyErrorCode as D, AuthAction as Dt, PasskeyError as E, AiAction as Et, cognitoUserPoolsTokenProvider as F, parseAmplifyConfig as Ft, createUserPoolDeserializer as G, createInitiateAuthClient as H, HubInternal as Ht, tokenOrchestrator as I, assertIdentityPoolIdConfig as It, getAuthUserAgentValue as J, assertServiceError as K, DefaultOAuthStore as L, assertOAuthConfig as Lt, signInStore as M, __assign as Mt, dispatchSignedInHubEvent as N, __rest as Nt, assertPasskeyError as O, Category as Ot, cacheCognitoTokens as P, __spreadArray as Pt, assertDeviceMetadata as Q, getAuthStorageKeys as R, assertTokenProviderConfig as Rt, deserializeJsonToPkcCreationOptions as S, parseMetadata as St, handlePasskeyError as T, setCustomUserAgent as Tt, DEFAULT_SERVICE_CLIENT_API_CONFIG as U, AmplifyErrorCode as Ut, createVerifySoftwareTokenClient as V, Hub as Vt, cognitoUserPoolTransferHandler as W, ConsoleLogger as Wt, assertAuthTokens as X, getCurrentUser as Y, assertAuthTokensWithRefreshToken as Z, setActiveSignInUsername as _, getRetryDecider as _t, getActiveSignInUsername as a, getRegionFromUserPoolId as at, getNewDeviceMetadata as b, parseJsonBody as bt, getSignInResult as c, AuthErrorCodes as ct, handleChallengeName as d, I18n as dt, AUTO_SIGN_IN_EXCEPTION as et, handleCustomAuthFlowWithoutSRP as f, defaultStorage as ft, handlePasswordSRP as g, composeTransferHandler as gt, handleUserSRPAuthFlow as h, unauthenticatedHandler as ht, assertUserNotAuthenticated as i, getRegionFromIdentityPoolId as it, setActiveSignInState as j, StorageAction as jt, passkeyErrorMap as k, GeoAction as kt, getSignInResultFromError as l, validationErrorMap as lt, handleUserPasswordAuthFlow as m, getDnsSuffix as mt, openAuthSession as n, SETUP_TOTP_EXCEPTION as nt, getMFAType as o, assertValidationError as ot, handleCustomSRPAuthFlow as p, AmplifyUrl as pt, createUserPoolSerializer as q, getRedirectUrl as r, SignUpException as rt, getMFATypes as s, AuthError as st, signInWithRedirect as t, OAUTH_SIGNOUT_EXCEPTION as tt, getTOTPSetupDetails as u, AuthValidationErrorCode as ut, retryOnResourceNotFoundException as v, jitteredBackoff as vt, getIsPasskeySupported as w, getAmplifyUserAgent as wt, assertCredentialIsPkcWithAuthenticatorAttestationResponse as x, parseJsonError as xt, getUserContextData as y, composeServiceApi as yt, createCognitoUserPoolEndpointResolver as z, decodeJWT as zt };
